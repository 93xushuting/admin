<template>
  ...
  <zp-page :obj="obj" ref="page">
    <zp-tabsCustom slot="tabs" v-model="tagName" :data="obj.tabs" />
    <!-- 查询条件 -->
    ...
    <!-- 加载表格数据 -->
    <zp-dynamicTable slot="table" :obj="obj.tableObj" stripe v-if="tagName === 'meSetting'" style="margin-top: 20px;">
    ...
    <information v-else ref="information" />
    <!-- 查看 -->
...
<script>
import information from '@v/reminder/information'
export default {
  name: 'choice',
  components: { information },
  data () {
    return {
      tagName: this.$route.query.id ? 'list' : 'meSetting', // tabs 默认选中的值
      dialogTip: {},
      obj: new _baseData().call(this, () => {
        return {
          tabs: [
            { value: 'meSetting', label: '消息设置' },
            { value: 'list', label: '消息列表' }
          ],
          http: {
            queryUrl: '',
            updateUrl: '',
  ...
  created () {
    ...
    if (!(window.btns['1413317741917638656'] && window.btns['1413317741917638656']['stationMessage/getStationMessageSetList'])) {
      this.obj.tabs = [
        { value: 'list', label: '消息列表' }
      ]
      this.tagName = 'list'
    } else {
      this.obj.http.queryUrl = 'stationMessage_getStationMessageSetList'
    }
  },
  mounted () {
